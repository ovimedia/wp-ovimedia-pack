...
<div id="floating_widget_container" class="tab_content">
    ...
    <select id="floating_widget_icon" name="floating_widget_icon" >

    <?php

    $faicons = get_font_awesome_icons();
    
    foreach ($faicons as $icon => $unicode)
    { 
        echo "<option value='".$icon."' ";                             

        if($this->wop_options['floating_widget_icon'] == $icon) 
            echo ' selected="selected" '; 

        echo "> &#x".$unicode." ".substr($icon, 3)."</option>"; 
    } ?>
